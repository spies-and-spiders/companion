...
(def schemas
  ...
  {;; --- loot specifications (drive the generic input forms) ---
   ...
   ;; --- the :cli plugin's stdout contract ---
   ;; The "friendly", un-namespaced JSON a :cli command writes to stdout, mapped
   ;; to ::view-model in sns.builtin.cli. External authors validate against this
   ;; (their own keys), never the :loot/... view-model. `action` is a bare name
   ;; the adapter keywordises to route the follow-up back to the same plugin.
   ::cli-item     [:map
   ...
   ::cli-output   [:map
                   ...
                   [:sections {:optional true} [:sequential ::cli-section]]
                   [:actions {:optional true} [:sequential ::cli-action]]]
