...
(deftest capabilities-reflect-reporter
  (testing "no reporter -> no report capability"
    (is (= {:browser-storage? false :loot-die-size 100 :sections [:divine-dust] :loot-table [{:id :divine-dust :ranges [[1 100]]}]} (engine/capabilities (engine/create base-config)))))
  (testing "with a reporter -> report? and label surfaced"
    (let [eng (engine/create base-config {:reporter (recording-reporter (atom nil))})]
      (is (= {:browser-storage? false :loot-die-size 100 :sections [:divine-dust] :loot-table [{:id :divine-dust :ranges [[1 100]]}] :report? true :report-label "Send"}
